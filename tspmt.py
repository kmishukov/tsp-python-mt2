...
def make_branches(queue, best_solution_record):
...
if __name__ == '__main__':
    ...
    for _ in range(num_processes - 1):
        make_branches(initial_queue, best_solution_record)

    for n in range(num_processes):
        solution_for_process = initial_queue.dequeue()
        processes[n] = Process(target=mt_func, args=(solution_for_process, best_solution_record))
        processes[n].start()
